    ...
    <div style={s.page} className="page-wrap">
      ...
      <div style={s.tabs} className="tabs-row">
        {["all", "todo", "in_progress"].map((f) => (
          <button key={f}
            style={{ ...s.tab, background: filter === f ? "#7c3aed" : "#1e1e2e", color: filter === f ? "#fff" : "#9ca3af", border: filter === f ? "none" : "1px solid #374151" }}
            onClick={() => setFilter(f)}>
      ...
      {pageLoading ? (
        <div style={{ textAlign: "center", color: "#a78bfa", marginTop: 60, fontSize: 18 }}>
          ⏳ Loading tasks...
        </div>
      ) : filtered.length === 0 ? (
